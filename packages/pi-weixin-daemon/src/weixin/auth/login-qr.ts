...
async function pollQRStatus(
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      // Long-poll timeout: normal, keep waiting.
      return { status: "wait" };
    }
...
export type WeixinQrWaitResult = {
   ...
   * this daemon, so no new credentials are issued and existing local
   * credentials remain valid. Treat as a successful outcome.
   */
